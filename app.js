...
(function () {
  ...
  /* 전역 클릭 리스너 — 모바일·터치 백업 경로.
     ...
     단 응답 영역의 두 버튼은 단계 D-9에서 제거됨 — 키보드만으로 단순화. */
  ...
  /* 두 버튼(네, 시작하기/나중에)은 단계 D-9에서 제거됨 — btnYes, btnNo 참조 없음. */
  ...
  function askWelcomeQuestion() {
    ...
    /* 환영 멘트 끝나면 짧게 다음 행동 안내 — 사용자가 무엇을 해야 할지 명확히.
       백스페이스 안내는 환영 멘트에서 이미 했으므로 여기선 스페이스만 짚어줌.
       결과 화면 진입 안내와는 다른 짧은 멘트 — 같은 멘트 반복을 피함. */
    var nextStep = '오늘의 뉴스 기사를 보시려면 스페이스 키를 눌러주세요.';

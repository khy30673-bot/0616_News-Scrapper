...
(function () {
  ...
  var DetailScreen = {
    cancelled: false,
    readyForLLM: false,   // 전문이 다 끝나서 엔터가 의미를 갖는 상태인지

    onEnter: function () {
      this.cancelled = false;
      this.readyForLLM = false;
      var self = this;
      ...
      /* 진입 안내 음성 → 짧은 숨 → 전문 음성 → 끝 안내 음성·영역.
         끝 안내가 끝나면 readyForLLM = true → 엔터 키가 LLM 화면으로 이동시킴. */
      var introMsg = '전체 기사를 읽어드리겠습니다. 듣기를 중단하시려면 백스페이스 키를 눌러주세요.';
        ...
        setTimeout(function () {
          ...
          TTS.speak(article.body, function () {
            ...
            setTimeout(function () {
              ...
              TTS.speak(endMsg, function () {
                if (self.cancelled) return;
                /* 끝 안내 음성이 끝나야 엔터가 작동 — 사용자가 안내 다 듣고 결정하도록 */
                self.readyForLLM = true;
              });
            }, 400);
          });
        }, 600);
    ...
    /* 사용자가 detail 화면에서 엔터를 눌렀을 때 호출됨.
       전문 + 끝 안내가 다 끝나야(readyForLLM === true) LLM 화면으로 이동.
       그 전이면 무동작 — 사용자가 안내 다 들은 후에만 의미 있는 동작. */
    selectLLM: function () {
      if (!this.readyForLLM) return;
      if (window.speechSynthesis) window.speechSynthesis.cancel();
      goTo('llm');
    },
  ...
  /* detail 화면 키 동작:
     - 엔터 = 전문 + 끝 안내가 다 끝났을 때만 LLM 화면으로 이동.
       그 전에 누르면 무동작 (DetailScreen.selectLLM 안에서 readyForLLM 체크).
